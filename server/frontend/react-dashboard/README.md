...
단일 챔버의 센서값, 공기질 상태, 자동 환기 팬, 차트, 최근 활동과 환기 효과를 보여주는 React 대시보드입니다. 로그인과 대시보드 데이터는 C# 백엔드 API를 사용합니다.
...
기본 프론트엔드 주소는 `http://localhost:5173`입니다. 로그인과 대시보드는 `VITE_API_BASE_URL`에 설정한 백엔드를 호출합니다.
...
`VITE_API_BASE_URL`을 실행 중인 백엔드 주소로 설정하고 Vite를 재시작합니다.

```env
VITE_API_BASE_URL=http://localhost:5026
```

현재 `POST /api/login`, `GET /api/dashboard`, `GET /api/dashboard/history`가 실제 서버를 사용합니다. 요약과 상태 이벤트는 히스토리 응답에서 계산하며, 별도 경고 API가 없어 경고는 표시하지 않습니다.
...
| ---------- | ------------------------ | ---------------------------- |
| `history`  | `/api/dashboard/history` | 브라우저에서 최신값을 누적   |
| `summary`  | `/api/dashboard/summary` | 브라우저 이력으로 계산       |
| `events`   | `/api/dashboard/events`  | 브라우저 이력에서 추정       |
| `alerts`   | `/api/alerts`            | 경고를 표시하지 않음         |

구현되지 않은 API의 capability는 반드시 `false`로 둡니다. capability를 바꾼 뒤 Network 탭에서 선택한 요청의 HTTP 200, CORS, 갱신 시각을 확인합니다.
...
테스트는 공기질 경계, 평탄·중첩 응답 정규화, sessionStorage, Provider fallback, 차트·CSV, 개발용 Mock Store, 최근 활동과 환기 분석을 검증합니다.
...
- 이력 capability가 꺼져 있으면 브라우저 메모리의 최근 60개만 사용하고 새로고침 시 사라집니다.
